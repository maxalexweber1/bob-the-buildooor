...
| 4.13 ✅ | **Sign + submit 2 chained txs** (CIP-103) — *passed on preview 2026-07-23; both confirmed in block 4499951, `e8ef5cc2882d…` then `0086fae400f0…`, the latter spending the former's output* | **ONE** approval titled "Bulk signature request" listing **both** txs in full; tx #2 is labelled "Chained: spends output(s) of transaction #1" and shows its input resolved (not "unresolved"); approve → both submitted in order, two hashes logged, both confirm | **T6.5** |
